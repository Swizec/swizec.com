...
```typescript
// A union type for field types
export type FieldTypes = TextFieldProps | EmailFieldProps;

// All fields share some properties
export interface CoreFieldProps {
    name: string;
    initialValue?: string;
}

// Text fields require a placeholder
// type is `text` by default
export interface TextFieldProps extends CoreFieldProps {
    type?: "text";
    placeholder: string;
}

// Email fields don't require a placeholder
export interface EmailFieldProps extends CoreFieldProps {
    type: "email";
    placeholder?: string;
}
```
...
```typescript
const RenderField = (props: FieldTypes) => {
	if (typeof props == TextFieldProps) {
		// render a text field
	}else if (typeof props == EmailFieldProps) {
...
```typescript
function isDuck(animal: Duck | Elephant | Donkey) {
	return "swim" in animal && "fly" in animal;
}
```
