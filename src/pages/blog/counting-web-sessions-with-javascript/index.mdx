...
![GA Count of Sessions view](https://swizec.com/wp-content/uploads/2017/05/histogram_screenshot.jpg "GA Count of Sessions view")
...
![Local storage screenshot](https://swizec.com/wp-content/uploads/2017/05/localstorage_screenshot.png "Local storage screenshot")
...
The first condition checks if 30 minutes have passed since the last update, the second checks if the date has changed, and the third check if the `lastUtmCampaign` is different from the `currentUtmCampaign`. `.some(b => b)` returns `true` if any of the conditions are truthy.
