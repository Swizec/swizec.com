...
![An air-raid siren in Nice, France, is still op...](https://swizec.com/wp-content/uploads/2011/12/300px-Nice_air-raid_siren2.jpg)
...
In [JavaScript](http://en.wikipedia.org/wiki/JavaScript) that's the only way you can do it. I shit you not.
...
But when I went looking for an elegant way of coercing datetime into a [string](http://en.wikipedia.org/wiki/String_%28computer_science%29), I found the whole situation rather lacking. Turns out JavaScript doesn't have a native [strftime](http://en.wikipedia.org/wiki/Time.h) function even though it is surprisingly brilliant in converting strings to native representations.
...
Pretty much the only date to string conversions javascript natively supports is returning a locale string, which is nonstandard and varies greatly computer to computer, and forms of standardized [ISO time](http://en.wikipedia.org/wiki/ISO_8601). It can do this and only this in numerous ways even!
