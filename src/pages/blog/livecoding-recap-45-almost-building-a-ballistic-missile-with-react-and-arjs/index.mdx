...
![Here he is next to Kiwi](https://swizec.com/wp-content/uploads/2017/09/ar-stuff-1024x576.jpg)
...
```javascript
<a-entity minecraft={`heightMeter: 2; skinUrl: ${skinUrl}`} minecraft-head-anim="yes" minecraft-body-anim="hiwave">
</a-entity>
```
...
class BallisticMissile extends Component {
    ...
    render() {
        ...
        return (
            <a-entity geometry={`primitive: 'box', width: .2, height: 1, depth: .2`} material={`roughness: 0, src: require('. texture.jpg'}`} scale={`x: y: z: 1`} position={pos}
...
This did not work. I couldn't figure out how to set a tick handler that actually gets called, so we set up a little game loop in our `App` component to make the missile fly.
