...
As luck would have it, last night I was hit over the head with my own words and nearly died [debugging](http://en.wikipedia.org/wiki/Debugging) a single function.
...
Due to poor decoupling - I didn't really want to decouple a 6 line function into two functions - everything was now difficult to test. I can't create events on Stripe's servers with [unit tests](http://en.wikipedia.org/wiki/Unit_testing) and without actual events existing I can't test the function works as it's supposed to.

How many bugs can you put in a 6 line function anyway?

## A lot of bugs!

When the client tested on staging ... it didn't work. Invoice email wasn't sent and Stripe complained of a [500 error](http://en.wikipedia.org/wiki/List_of_HTTP_status_codes).

![English: British Army Signals Trials Unit test...](https://swizec.com/wp-content/uploads/2012/02/300px-Army_Camp-sml.jpg)
...
With proper [unit testing](http://en.wikipedia.org/wiki/Unit_testing) the debug cycle would look like this:
...
Yes, all of those could _easily_ have been caught if my [test coverage](http://en.wikipedia.org/wiki/Fault_coverage) was actually any good! And then not only would I not look like an idiot in front of the client, I'd probably spend no more than ten minutes fixing this.
