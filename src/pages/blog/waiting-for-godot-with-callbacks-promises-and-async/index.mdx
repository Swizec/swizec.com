...
Look at this code. Do you know what it prints? ![What does it print](https://swizec.com/wp-content/uploads/2018/11/doesitprint-1024x562.png)
...
![waiting](./img/media-iAYupOdWXQy5a4nVGk-giphy.gif)
...
What does this print? ![What does it print](https://swizec.com/wp-content/uploads/2018/11/whatprint2-2.png) Hard to follow right? It's `5` then `3`.
...
Leads to callback hell when you run a bunch of stuff in sequence. ![Callback Hell](https://swizec.com/wp-content/uploads/2018/11/callbackhell.png) I always liked this kinda code. It's clear, readable, goes down the steps linearly. Super nice.

Error handling makes it harder. That usually worked like this: ![Error Handlings](https://swizec.com/wp-content/uploads/2018/11/errorhandlig.png) This gets messy. Making sure you handle all errors correctly and your UI shows what's up is hard. Super messy, super fast.
...
Like this ![Promises Example](https://swizec.com/wp-content/uploads/2018/11/waitforgodot-1024x605.png) Some say this is easier to read. I think it's about the same. The flat structure does look nice.

You could write that same code like this: ![Another Promises Example](https://swizec.com/wp-content/uploads/2018/11/waitforgodot2-1024x791.png) _Anything_ you return from a promise is wrapped in a promise. If you return a promise, it gets flattened. You don't need `.then.then` to unpack the value.

Observe 👇 ![Promises Example](https://swizec.com/wp-content/uploads/2018/11/waitforgodot3-1024x871.png) Print directly, and you get a `Promise` object. Mind you, it still executes. A promise doesn't care if you ever use the result. It executes right away.
...
We can make our wait faster with `Promise.all`. That lets us run promises in parallel and deal with their results all at once. ![Promises with Promise.all](https://swizec.com/wp-content/uploads/2018/11/waitforgodot4.png) 👌
...
Waiting for Godot looks like this: ![Async/Await Example](https://swizec.com/wp-content/uploads/2018/11/await1.png) You now have code that's extremely linear. Reads just like normal imperative code you know and love.
...
Either way, you can get values out of an async function using await just like you would with a Promise chain. Every async is await-able. ![Async/Await Example](https://swizec.com/wp-content/uploads/2018/11/await2-1024x392.png) But you can't `await` outside an async function. That's why I had to wrap that code.
...
But it does lead to some cool things. Like easy iteration: ![Async/Await Example](https://swizec.com/wp-content/uploads/2018/11/await3.png) You can use `await` to wait for values inside a loop. The next iteration happens only after your Promise resolves.

And if you want to wait in parallel, you can do that, too. Just use `Promise.all` as before. ![Async/Await Example](https://swizec.com/wp-content/uploads/2018/11/await4-1024x421.png) Remember, `Promise.all` returns a promise and `await` can wait for any promise.
...
![Savvy Gif](https://l56spuxk1t14k3seci6s2bja-wpengine.netdna-ssl.com/wp-content/uploads/GIF/2014/10/Savvy-GIF.gif)
