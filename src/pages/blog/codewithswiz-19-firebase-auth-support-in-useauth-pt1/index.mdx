...
[Video Link](https://www.youtube.com/watch?v=qMtaaI-xjJM)
...
Fill in those methods to fit your auth provider. Here's what we tried for `FirebaseAuth`. Don't know yet if it works.
...
Render `<AuthConfig>` and provide the new auth provider. We don't know yet which params `FirebaseAuth` is going to use. Find out next time :)
