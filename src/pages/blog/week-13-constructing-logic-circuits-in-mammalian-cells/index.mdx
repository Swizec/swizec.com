...
The paper uses this magic to build different logic gates. NOR and NAND in particular because they are functionally complete and can be used to implement anything. You could build a whole computer just by combining NOR or [NAND gates](http://en.wikipedia.org/wiki/NAND_gate).
...
![Design of single-layer genetic NOR gates](./img/images-7919.png)
...
The first step towards a complex circuit was implementing a single [NOR gate](http://en.wikipedia.org/wiki/NOR_gate) as an OR-[NOT gate](http://en.wikipedia.org/wiki/Inverter_%28logic_gate%29) with three layers - two layers for OR and an extra inverter to make a NOR.
...
To make a simple [OR gate](http://en.wikipedia.org/wiki/OR_gate) they place two binding sites for different TALE repressors before the [constitutive promoter](http://en.wikipedia.org/wiki/Promoter_%28genetics%29) followed by an expressor gene. To turn it into a NOR gate, they added another binding site next to the other two.
...
The real test of orthogonality comes when you want to implement a multi-layered [logic circuit](http://en.wikipedia.org/wiki/Logic_gate) - an [XOR gate](http://en.wikipedia.org/wiki/XOR_gate).
...
![An XOR gate](./img/images-7992.png)
...
![Majority function](./img/Majority-function23h111.png)
