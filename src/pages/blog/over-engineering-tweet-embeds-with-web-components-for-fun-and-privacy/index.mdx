...
To get around that issue, `<twitter-lite>` assumes your site runs a light endpoint at `/api/fetch-tweet`. Easy to do with both Gatsby and NextJS by putting a bit of code in your `/api/` directory. It then runs as a serverless function.
...
I haven't tested with NextJS, but on Gatsby deployed to Gatsby Cloud, you can put that whole file in `/api/fetch-tweet` and it Just Works 🤘
