...
[Webpack](http://webpack.github.io/)'s core premise is that you can `require('./foo')` your [JavaScripts](http://en.wikipedia.org/wiki/JavaScript). That sea of `<script>` tags or concatenated code and variables injected into the global namespace is gone.
...
[Browserify](http://browserify.org/) was better. Using [CommonJS](http://en.wikipedia.org/wiki/CommonJS) it brought the same syntax we've had in node.js into the browser. Less clunky and easier to reason about. But to my knowledge, not all that popular in the wild.
...
The `in_dev()` and `getCDN()` functions make the config easier to understand. They look like this:
