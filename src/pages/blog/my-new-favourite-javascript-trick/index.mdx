...
Using returns and [callbacks](http://en.wikipedia.org/wiki/Callback_%28computer_programming%29) in the same function.

Sounds like crazy talk I know, but hear me out, I have good reason. I think.

Let's say you want to make several [TCP](http://en.wikipedia.org/wiki/Transmission_Control_Protocol) servers in a node.js application. Have to listen on multiple ports or whatever. Using a factory function is your best bet to avoiding code repetition, right?
...
Not much has changed. More things were bound to `this` and the [return value](http://en.wikipedia.org/wiki/Return_statement) became a function because I feel [partial application](http://en.wikipedia.org/wiki/Partial_application) makes this code cleaner.
...
Another trick I sneaked into these examples is [Javascript](http://en.wikipedia.org/wiki/JavaScript)'s powerful [dynamic scoping](http://en.wikipedia.org/wiki/Scope_%28computer_science%29). Getting semi-random functions to run in the same scope like that can really clean up your code.
