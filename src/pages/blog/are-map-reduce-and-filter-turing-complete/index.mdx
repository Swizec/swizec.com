...
To have a [turing complete language](http://en.wikipedia.org/wiki/Turing_completeness) you need:
...
In the context of a strict [functional language](http://en.wikipedia.org/wiki/Functional_programming), with the addition of recursion, yes we probably can.

After all, implementing a naive [quicksort](http://en.wikipedia.org/wiki/Quicksort) in [Haskell](http://haskell.org) takes nothing more than recursion, `car`, `cdr`, `cons` and filter.
...
We can implement that Haskell example in [Javascript](http://en.wikipedia.org/wiki/JavaScript) as well. Proving that even with Javascript recursion, map, and reduce are enough to do anything.
...
[Enhanced by Zemanta](https://img.zemanta.com/zemified_e.png?x-id=5af887dc-447d-4f5a-98dc-08047cfee16f)