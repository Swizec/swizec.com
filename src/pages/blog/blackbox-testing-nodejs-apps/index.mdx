...
One of the best features in [Django](http://www.djangoproject.com) is the test client that comes with the test framework -> blackbox testing baby, yeah!

![Black box flight recorder](https://farm3.static.flickr.com/2215/1972546648_e63db0846e_m.jpg)

[Blackbox testing](http://en.wikipedia.org/wiki/Black-box_testing) is my favourite kind of testing. Much more useful than [unit tests](http://en.wikipedia.org/wiki/Unit_testing) and generally a lot easier to write, maintain and deal with. And that's because rather than making sure all your functions and deep internals are behaving like they should, you only test what really matters in a web app.
...
1.  Low maintenance tests - you don't have to rewrite the whole [test suite](http://en.wikipedia.org/wiki/Test_suite) when refactoring the deep internals of your app
...
Where `{app}` is, for example, an express.js server - `{request}` is our test client. We are just checking that sending a [GET request](http://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol) to _/user_ will return some sort of json and that the status code will be 200.
