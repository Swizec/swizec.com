...
![Visual vs. Densitometric segmentation (expected)](./img/KwdOq.png)
...
Analyzing the different implementations of article extractors reveals that far from using a methodical approach of marking up different bits of a page, they mostly work as tree [pruning algorithms](http://en.wikipedia.org/wiki/Pruning_%28decision_trees%29) - go through [DOM](http://en.wikipedia.org/wiki/Document_Object_Model), remove anything that's not promising, end up with the juicy article.
...
![Visual vs. Densitometric (actual)](./img/LCupU.png)
