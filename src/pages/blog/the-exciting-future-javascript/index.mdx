...
`[ECMAscript](http://www.ecma-international.org/publications/standards/Ecma-262.htm)`, 6th edition might bring us fat arrow notation - `[Douglas Crockford on fat arrows](http://www.yuiblog.com/blog/2012/03/30/what-is-the-meaning-of-this)`
...
Douglas' post concerns itself mostly with the intricacies of how _this_ is bound to the `[function object](http://en.wikipedia.org/wiki/Function_object)`and what the fat arrow notation might bring, what I see is something completely different.

![vim](./img/4073-4766782412_f471ddd85e_m.jpg)

Proper lambdas!

That's right, `[functions](http://en.wikipedia.org/wiki/Function_%28mathematics%29)` that implicitly return with none of that big ugly _function_ keyword! Hooray.
...
As I said on hacker news _`[Javascript](http://en.wikipedia.org/wiki/JavaScript)` is a beautiful language, trapped in bad syntax, peppered with poor semantic choices_. What is left of a language once you take syntax and semantics out of the picture? Quite a lot.
...
`[Iterators](http://en.wikipedia.org/wiki/Iterator)` are a similar beast - you get to define _how_ an object should be iterated over, which gives us nice abstractions for all sorts of things.
