...
Everybody loves [code completion](http://en.wikipedia.org/wiki/Autocomplete). You type three characters instead of ten and save some time.

Like, almost a second.

Being glued to a keyboard most of your life means you can type at some 100 [words per minute](http://en.wikipedia.org/wiki/Words_per_minute). With the average English word's length of 5.1 characters, this gives you 8.5 characters a second
...
On Sunday I was adding some [JavaScript](http://en.wikipedia.org/wiki/JavaScript) to a webpage using an [Emacs](http://www.gnu.org/software/emacs) mode that does simple code completion. When you write `</` it fills out the rest.
...
_Wdiget_. A [typo](http://en.wikipedia.org/wiki/Typographical_error). Honest mistake, happens all the time. But code completion replicated the typo all over the place.
