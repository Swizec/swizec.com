...
const Luke = () => {
	...
	return loading ? 
		<p>Fetching ...</p> 
	: 
		<p>{data.Person.name} is a {data.Person.hairColor[0]} {data.Person.species[0].name} and flies around in {data.Person.starships.map(ship => ship.name).join(", ")}</p>
}
