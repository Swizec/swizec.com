...
I know! Let's make a bot that does this! A low frequency trading bot, that sounds like fun. And let's write it in [Haskell](http://haskell.org), just to keep things interesting. Marrying the strictest of languages with the messiest of resources - the internet ... what could possibly go wrong?

First order of business - a REST client.

## REST client

Before my bot can do any trading and intricate [algorithmic trading](http://en.wikipedia.org/wiki/Algorithmic_trading), it needs to talk to the marketplace of choice. I picked Bitstamp because they're the only ones that let me do this without a US bank account.
...
First of all, we are going to need a bunch of imports. The ones we _really_ care about are the _http-conduit_ library and the _Aeson_ parser of [JSON](http://json.org/) strings. Everything else is there because ... well I'm not sure actually, but it seems to be necessary, otherwise things don't work.
...
All it takes now is making an [HTTP request](http://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol) and parsing the response as JSON. Simple, right?
...
Great. We have a _Ticker_ type that has a bunch of numbers and some names. The \_Show \_part seems to say that we'll be able to print this out to the console. Smashing!
...
But hey! Got Haskell to talk to a [REST API](http://en.wikipedia.org/wiki/Representational_state_transfer). How cool is that!?

![Enhanced by Zemanta](https://img.zemanta.com/zemified_e.png?x-id=5492ac2c-3abf-45a3-9680-dd8c31c07973)