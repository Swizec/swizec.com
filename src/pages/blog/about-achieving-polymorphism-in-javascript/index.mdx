...
After you authenticate - testing [OAuth](http://oauth.net) clients sucks, and Toshl invalidates your token _very_ often. Think I had to refresh it four times in a five hour coding spree. - you can talk to `https://api.toshl.com/expenses` to get a list of expenses for the current user.
...
Let's say you have a function called `toshl.expenses`, in [Haskell](http://haskell.org) you could do something like this:
...
Here's that same polymorphic code in [Javascript](http://en.wikipedia.org/wiki/JavaScript). This time tested working code.
