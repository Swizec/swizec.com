...
![DRY Magic](./img/media-12NUbkX6p4xOO4-giphy.gif)
...
const CloseModal = () => (
    <GenericButton onclick={`{closeModal}`} label="Close Modal" style={{ background: 'red' }} />
)
```
...
![A-ha moment](./img/media-d3mlE7uhX8KFgEmY-giphy.gif)
...
const GenericButton = ({ onClick, label, active, style }) => (
  <button onclick={onClick} disabled={`{!active}`} style={{ fontSize: 20, ...style }}>
    {label}
  </button>
);
```
...
![Excitement](./img/media-ibGFpMv1Uoais-giphy.gif)
