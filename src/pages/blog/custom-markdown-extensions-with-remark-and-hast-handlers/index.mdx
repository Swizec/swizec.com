...
```undefined
<code class="javascript">import visit from "unist-util-visit";
...
```undefined
<code class="javascript">function getScreenshotUrl(url) {
    return `https://84wz7ux5rc.execute-api.us-east-1.amazonaws.com/default/screenshot-as-a-service-dev-screenshot-function?url=${url}`;
}
</code>
```
...
```undefined
<code class="javascript">function remarkUrlThumbnail({ domains = [] }) {
    return tree =>
        new Promise(async (resolve, reject) => {
            const nodesToChange = [];
            visit(tree, "link", node => {
...
```undefined
<code class="javascript">const screenshotHandler = (h, node) => {
    const props = { node };
...
```undefined
<code class="javascript">import remark from "remark";
...
```undefined
<code class="javascript">.use(remark2react, {
    sanitize: false,
    remarkReactComponents: {
        screenshot: Screenshot
    },
