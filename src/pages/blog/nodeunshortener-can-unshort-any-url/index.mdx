...
![Twitter Inc HQ](./img/Twitter-Inc-HQjgh1bc.jpeg)
...
That project deals mostly with [Twitter](http://twitter.com) and harnessing url's from the stream. As anyone who's sniffed at Twitter lately can atest, therea re many, many _many_ strange urls floating around, using any number of [url shortening](http://en.wikipedia.org/wiki/URL_shortening) schemes or what seems to have become quite fashionable, a custom domains that are shortenered via services nobody could guess at.
...
- know it --> use an [API](http://en.wikipedia.org/wiki/Application_programming_interface)
- wtf --> pretend to be a browser

As you can see, the library is pretty simple, just 68 sloc of [JavaScript](http://en.wikipedia.org/wiki/JavaScript) code. But simple solutions are good solutions I'm told and this little thing does everything I need it to do.
