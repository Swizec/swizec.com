...
Screw the mathematical definition. Look at this 👇
...
That means our `getQuestion` function first constructs `Earth`, then gets `ArthurDent`, accesses his subconsciousness, and uses Scrabble™ to print the result. 🤙
...
getPaypalPaymentInfo(){
    return `this.brainTreeClient.tokenizePaypal()`
                         .then(response => {
