...
![vanilla dropdown gif](https://swizec.com/wp-content/uploads/2016/11/vanilla-dropdown.gif)
...
const field = ({ options }) => (
    `<Select name="university" value="one" options={options} onchange="{val}" ==""> console.log(val)</Select>`
);
```
...
![react virtualized select gif](https://swizec.com/wp-content/uploads/2016/11/react-virtualized-select.gif)
...
const field = ({ options }) => (
    `<Select name="university" value="one" options={options} filteroptions={filterOptions} onchange="{val}"> console.log(val)</Select>`
);
```
