...
![Default bootstrap example - zoomable etc.](https://swizec.com/wp-content/uploads/2012/11/default-bootstrap-example.png)
...
- renders super small despite the `{_initial-scale=1.0_}` [meta tag](http://en.wikipedia.org/wiki/Meta_element)
...
Default zooming is okay now and users can't [pinch-to-zoom](http://en.wikipedia.org/wiki/Multi-touch_gestures) anymore. But they can still do plenty of strange side-scrolling and bouncing around. Something apps don't usually do.
...
Our only problem now is that pesky [address bar](http://en.wikipedia.org/wiki/Address_bar) on the top. Apps don't have that! This can be solved with a bit of simple javascript:
...
![Final result - an app-like website](https://swizec.com/wp-content/uploads/2012/11/app-like-bootstrap-example.png)
