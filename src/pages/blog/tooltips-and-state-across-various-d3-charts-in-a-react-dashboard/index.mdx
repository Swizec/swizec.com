...
Then we render the `Tooltip` component.

## Third, you need a Tooltip

The `Tooltip` component takes coordinates and content. Renders them in an SVG `foreignObject` so we can leverage HTML's built-in text formatting abilities.
