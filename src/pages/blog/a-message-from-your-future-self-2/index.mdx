...
1.  Smooth out the data with a [rolling average](http://en.wikipedia.org/wiki/Moving_average) to get a curve
2.  Calculate an [expected value](http://en.wikipedia.org/wiki/Expected_value) for the next day after known data
...
Weights are calculated according to the simplest [infinite series](http://en.wikipedia.org/wiki/Series_%28mathematics%29) I could think of that always sums to one no matter how many elements are needed. But I also split it in half and shuffle it about at the pivot, so the strongest weight is moved back in the data when calculating days. So in a sense, when calculating the 1st unknown value, the last known has highest weight. Then the penultimate known and so forth.

The algorithm is pretty simple in [Haskell](http://haskell.org)
