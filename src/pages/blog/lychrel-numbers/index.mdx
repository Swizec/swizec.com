...
Last night I discovered another cool mathematical concept akin to the [Collatz conjecture](http://en.wikipedia.org/wiki/Collatz_conjecture) - [Lychrel numbers](http://en.wikipedia.org/wiki/Lychrel_numbers).

[Collatz map fractal in a neighbourhood of the ...](https://swizec.com/wp-content/uploads/2012/01/300px-CollatzFractal4.png)

The idea of a lychrel number is pretty straightforward: Take a number, add its reverse, continue until you reach a [palindrome number](http://en.wikipedia.org/wiki/Palindromic_number). If you never reach a [palindrome](http://en.wikipedia.org/wiki/Palindrome), then this is a [Lychrel number](http://en.wikipedia.org/wiki/Lychrel_number).
...
Honestly I am not certain what class of problems lychrel numbers fall into. The "not a lychrel number" is a half-[decidable problem](http://en.wikipedia.org/wiki/Decision_problem). It will always tell you when _a number is not lychrel_ but it will never terminate when it is. If my understanding is correct, this would make "is a lychrel number" an non-decidable problem.

[Project Euler](http://projecteuler.net/) is kind enough to limit the problem a little bit and make it a fun algorithm to write before bed when your brain is half dead. _Find all lychrel candidates under 10,000 assuming it should never take more than 50 iterations._

Solving _that_ problem becomes rather trivial in [Haskell](http://haskell.org):
