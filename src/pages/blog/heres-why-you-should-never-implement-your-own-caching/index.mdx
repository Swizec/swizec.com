...
1. Fetches the `latest` app version
   1. `{latest}` returns saved value if it exists
   2. if not, `{latest}` saves its result in a class instance variable
2. If we got a version, we check if the new app is of a newer version
3. If user's device is newer, we create a new `{AppVersion}` entry
4. Then we update the class instance variable
5. If device is not newer, we update the `{last_seen_at}` timestamp
6. If this is the first time ever that we're checking – there's no `latest` – then we make a new latest and update the class instance variable
