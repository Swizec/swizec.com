...
And until a few days ago, those categories appeared as useless plaintext:
...
Add a new function call to the `createPages` hook in `gatsby-node.js`. This lets you hook into the page creation part of Gatsby's build process.
...
Here are the steps for the `createCategoryPages` function:
...
Gatsby revolves around the idea of using GraphQL for accessing everything. And they mean _everything_. Even markdown files in your project.
...
This GraphQL query finds all [MDX files](https://www.gatsbyjs.com/docs/glossary/mdx/) under the `/blog/` or `/articles/` paths and extracts the `categories` field from their frontmatter. If there's an error, we throw it.
...
const allCategories = new Set(
  result.data.allMdx.nodes
    .filter((node) => Boolean(node.frontmatter.categories))
...
First, a filter clears empty strings. Second, a map extracts the `categories` property. Third, a map splits by comma. Fourth, a `.flat()` method flattens the array of arrays. Fifth, a map trims leading and trailing whitespace and lowercases the categories.

Now we're ready to create pages.
...
Each page will live on a `/categories/*` path using the category name. It will render with the `category.js` React component and will receive the `category` and a `categoryRegex` as its props.
...
The component is straightforward. It has some text, email signup forms, and an `<ArticleListing>` component that iterates through a list of articles and renders titles with descriptions.
...
We're using [Gatsby's support for page queries](https://www.gatsbyjs.com/docs/how-to/querying-data/page-query/) to get article data into the `data` prop at build time. On every page deployment.
...
We use that to `filter` all pages on the site by whether they include the category in their frontmatter. We also filter by file path to ensure only blogs or articles are included.
...
Adding links to categories is easy. You split the category string by comma and render links in a loop.
