...
_"Hey, so nobody's looked at our [Mixpanel](http://www.mixpanel.com) integration in months and the product has changed a lot, so uh ... can you fix that?"_
...
In my case, I had to fix an old javascript integration of Mixpanel. Our approach was a mix of globally assigning [event listeners](http://en.wikipedia.org/wiki/Observer_pattern) and peppering our view/model/whatever code with `{mixpanel.track}` calls.
...
Therefore, the only solution is moving tracking code an [abstraction layer](http://en.wikipedia.org/wiki/Abstraction_layer) higher. Instead of relying on everyone in the team to maintain tracking calls, move them into the framework they're using.

For us it's Backbone. So last night I wrote a simple 30 line [mixin](http://en.wikipedia.org/wiki/Mixin) for Backbone views that makes sure we track _everything_.
...
The code wraps and is heavily inspired by `{Backbone.View.delegateEvents}`, which is called when a new view is instantiated. Its job is to go through the events configuration hash and bind the view's event listeners to correct events on particular [HTML elements](http://en.wikipedia.org/wiki/HTML_element).

Nifty.

My mixin does the same, but only cares about `click` and `submit` events since those represent most user actions. Every such event is bound to a simple `track` function, which takes the `data-mixpanel` property from the element as an event's business name, and uses the view's whole model as [meta data](http://en.wikipedia.org/wiki/Metadata).
...
[Enhanced by Zemanta](https://img.zemanta.com/zemified_e.png?x-id=238011f3-8be3-4b29-a1af-b7f9ac498401)
