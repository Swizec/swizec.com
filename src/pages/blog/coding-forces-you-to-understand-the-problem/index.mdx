...
[name|Friend], you know how a pen works, right? A nice ballpoint pen that you use every day. 
...
![DST around the world – Blue and Orange observes DST – Light Grey used to observe DST – Dark Grey never had DST](https://upload.wikimedia.org/wikipedia/commons/thumb/1/16/DST_Countries_Map.png/2560px-DST_Countries_Map.png)figure}]
...
function getTimes(startAt, N, timezone) {
	...
	for (let i = 0; i < N; i++) {
		times.push(
			zonedTimeToUtc(addDays(zonedStartAt, 7 * i), timezone)
		)
	}
...
![Email validation state machine](https://i.stack.imgur.com/YI6KR.png)

Yeah, I never would've guessed it either.

[name|] Next time you look at convoluted code and think _"wow terrible, needs a rewrite"_, take a moment to consider you're missing some nuance. 
