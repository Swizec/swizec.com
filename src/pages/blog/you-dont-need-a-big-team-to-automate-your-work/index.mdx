...
![giphy: fillion_speechless](giphy:fillion_speechless)
...
![giphy: wonderful](giphy:wonderful)
...
Mechanically iterate through the list of commands, use `{execSync}` to run them in your desired directory – `{cwd}` – then use {chalk} to print a green success message.
