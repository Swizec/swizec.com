...
Not “big data” huge, more like _“Holy shit 5 years ago this would crash my browser”_ huge. Like, you’re playing with a dataset on 177,830 salaries[1](#fn1-30036) in the US software industry, and your computer doesn’t even notice until you do something stupid.
...
Not to mention the initial page setup. Loading and parsing the dataset (CSV) from cache takes about 3 seconds, running the initial `setState({data: foo})` takes about a second. That’s up to 5 seconds between every full page reload and seeing output.
...
Oh, and Firefox wins the overall _“How long do I have to wait before I can test my code?”_ challenge as well[2](#fn2-30036). You can shave a few milliseconds off the initial `setState({data: foo})` call.
