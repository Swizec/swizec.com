...
- [![Moss](https://swizec.com/wp-content/uploads/2010/08/2686845422_d924cc453e_m.jpg)](http://www.flickr.com/photos/50811886@N00/2686845422)

  Image by [warrenski](http://www.flickr.com/photos/50811886@N00/2686845422) via Flickr

Welcome to another installment of Swizec's [Django](http://www.djangoproject.com) protip. Previously we discussed a better way to [structure your django apps](https://swizec.com/blog/django-protip-1-a-better-app-structure/), but nobody cared about that because everybody is rather silly. This time we'll be talking about how awesome forms are and why you should be using them for pretty much everything. At the end, I'll show you some neat [tips and tricks](http://en.wikipedia.org/wiki/Tips_%26_Tricks) I discovered during my Django Epiphany.
...
Anyone notice a security flaw? Then think of this, what happens if the page is set to -1? Sure, if errors aren't being displayed right to the user nothing too important. But if they get shown an [SQL](http://en.wikipedia.org/wiki/SQL) error ... or worse ...

However people are usually at least a little bit more careful about POST data because they realise that hey, this is something a person filled in a [web form](http://en.wikipedia.org/wiki/Form_%28web%29) and perhaps the data should go through a series of a little bit more stringent tests before it gets chucked into the [database](http://en.wikipedia.org/wiki/Database). Hell, maybe we could even tell the user where they screwed up and if the planets are in constelation, why not also make sure those required fields are actually filled out ... you know, so we don't get any weird inconsistencies in our database.
...
But since most of my time is spent on developing [API](http://en.wikipedia.org/wiki/Application_programming_interface)'s rather than user-facing [websites](http://en.wikipedia.org/wiki/Website) let me show you how to use forms effectively in that sort of environment. For the html stuff just go check out the [django form docs](http://docs.djangoproject.com/en/dev/topics/forms/).
...
[![Enhanced by Zemanta](https://img.zemanta.com/zemified_e.png?x-id=c1b237d5-fb11-4a02-a61d-ebecc9071b07)](http://www.zemanta.com/)