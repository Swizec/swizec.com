...
Same `{ts-jest}` preset and `node` test environment as before, added `verbose` and `automock`. `{Automock}` is nice because it tells Jest to automatically create a mocked version of any imported code.
