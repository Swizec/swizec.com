...
The other week I was working on my compilers homework - the [semantic analysis](http://en.wikipedia.org/wiki/Compiler) part is an object-oriented nightmare. Something called the [visitor pattern](http://en.wikipedia.org/wiki/Visitor_pattern) to traverse trees and do weird stuff.

It made me want to curl up in a fetal position in the corner, rocking back and forth in a padded room, while mumbling tongues at myself. As [River Tam](http://en.wikipedia.org/wiki/List_of_Firefly_characters) would say before running out of the room in panic _"Too crowded!"_

## What [OOP](http://en.wikipedia.org/wiki/Object-oriented_programming) feels like

![Schizophrenic?](https://swizec.com/wp-content/uploads/2012/05/Summer_Glau_as_River_Tam_in_Serenity_Wallpaper__yvt2-300x225.jpg)

In object-oriented programming everything gets muddled together. Functions are bundled with data, everything is codependent and there's no telling what a [function](http://en.wikipedia.org/wiki/Function_%28mathematics%29) might do when you call it.
...
Answering this simple question requires knowing _**everything**_. The whole execution history of `{foo}`. The whole codebase. You name it, you have to keep it in your head.

Oh and did I mention class `{bar}`? It depends on `{foo}` for a lot of its stuff. Oh yeah, they got into a little friends with benefits situation last year. It all gets pretty interesting. I think `{bar}` cheated on `{foo}` with _[baz](http://en.wikipedia.org/wiki/Foobar)_ once, though. There's no telling how `{foo}` might react to that!

## Functional programming

![fp, by okeef creations](https://swizec.com/wp-content/uploads/2012/05/fp1.png)

With a lot of gentle poking and prodding from [@sbelak](http://twitter.com/sbelak), I started learning [functional programming](http://en.wikipedia.org/wiki/Functional_programming) about two years ago.
...
Sure, you could define a function called _X_ that combines _Y, Z, A _and _B_, but why create a new function for something that only gets called once? Much better to just do something like _X = A+B(Y(Z))_ isn't it?

## A waterfall of data

![Plitvice falls](https://swizec.com/wp-content/uploads/2012/05/waterfall-of-data.jpg)
...
You might say this is really [procedural programming](http://en.wikipedia.org/wiki/Procedural_programming), but I don't like side-effects, imperative bits of code and so on. It's still functional programming, but this is how my brain understands code. Your mileage might vary.
