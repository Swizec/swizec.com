...
[A DARPA challenge loser](https://swizec.com/wp-content/uploads/2011/12/ai-car-lol7.jpg)
...
Even though the world is a-buzz with news of a possible higgs boson discovery, I decided to start off Science Wednesday with excerpts from this week's lecture on robotics at ai-class.com. [Sebastian Thrun](http://en.wikipedia.org/wiki/Sebastian_Thrun) talked about Stanford's autonomous vehicles and how the Stanford Racing Team managed to snag victory at the DARPA challenge a couple years ago.
...
Two algorithms are essential to achieving this: localization and [path planning](http://en.wikipedia.org/wiki/Motion_planning).

### Localization

For localization they use something called [monte carlo localization](http://en.wikipedia.org/wiki/Monte_Carlo_localization). It uses a particle filter as the beating heart of the approach - it sounds all fancy, but it's really quite a simple approach.
...
For path planning two approaches are used. [Dynamic programming](http://en.wikipedia.org/wiki/Dynamic_programming) is used to plan the path and a modified [A\* algorithm](http://en.wikipedia.org/wiki/A%2A_search_algorithm) is used to plan the actual movement within the map.

[A* search that uses a heuristic that is 5.0(=ε...](https://swizec.com/wp-content/uploads/2011/12/Weighted_A_star_with_eps_58.gif)
