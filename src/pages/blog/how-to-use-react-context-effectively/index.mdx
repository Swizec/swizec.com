...
When rendering we use `Provider` to pass this component state as a context value down to our entire component tree.

### 3. Consuming context state

Consuming your state is a matter of rendering a `Consumer` and using the values it provides.
...
Yes, this approach works with hooks. Replace `Consumer` with `useContext`. Everything else stays the same.
