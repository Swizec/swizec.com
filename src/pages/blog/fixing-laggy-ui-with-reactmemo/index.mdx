...
The other day I was dealing with some laggy UI on a project and discovered React.memo as a way of fixing it. I don't know how long React.memo has been a thing but it's new to me and it's amazing.And when I say laggy UI, I mean _really_ laggy UI.
...
```undefined
<code class="javascript">{foo: 'bar'} === {foo: 'bar'} // false
</code>
```
...
```undefined
<code class="javascript">onChange: value => {
...
```undefined
<code class="javascript">export const MemoRecursive = React.memo(
  ({ values }) => {
    // render values
  },
  (prevProps, nextProps) =>
