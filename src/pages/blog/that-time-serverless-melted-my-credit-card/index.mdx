...
A `{reinforcement-learning}` approach that interpreted user actions on the fly. You could _see the code learn_. Point at your RSS feeds, read as normal, see your homepage change.
...
`{Google App Engine}` was all the rage and _definitely_ the future. `{BigTable}` was the hot new database offering with infinite scale.

![heck yeah](giphy:heck_yeah)
...
![download growth](./img/UnIt6Agif3hdejj.gif)
