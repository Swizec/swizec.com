...
![Accordion Flow Sketch](https://swizec.com/wp-content/uploads/2018/06/accordionflow-sketch.jpg)
...
Icon is a plain component. We're rendering it as a compound component with no frills. `DynamicContent` is wrapped in a function because it's going to be used as a render prop. It gets the callback function it can call to dynamically update its height after initial rendering.
...
Our `Row` component uses `state` to keep track of whether it's `open` and its vertical position `y`.
...
We use this opportunity to save the ref as a component property for future reference, get its height using `getBoundingClientRect`, and call the `reportHeight` callback to tell `AccordionFlow` about our new height.
